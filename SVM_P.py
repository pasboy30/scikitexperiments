...
for ds in list_of_datasets:
    ...
    svm_poly = svm.SVC(kernel='poly', C = 1.0 , degree = 2)
    count = 1
    final_results = 0
    for train_index, test_index in kf.split(X):
        ...
        pl.title("SVM - polynomial with degree '2' for 2 classes \n Dataset: " + str(ds) + "\n Split number:" + str(count))
        # pl.show()
        pl.savefig("SVM_L" + str(ds) + "split" + str(count) + '.png')
        count += 1
