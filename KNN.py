#Classification of Datasets using KNN 
...
for ds in list_of_datasets:
    ...
    for k_val in range(2,11):
        ...
        for train_index, test_index in kf.split(X):
            ...
            pl.title('KNN plot for ' + str(ds)+ ' where k = ' + str(k_val) +  ' \n Split number = ' + str(count))
            pl.savefig('KNN'+str(ds)+'k='+str(k_val)+'split='+str(count)+'.png')
            count += 1
        print("Confusion matrix (Aggregated) for Data-set " + str(ds) + "m where k = " + str(k_val)+" is: \n" + str(final_results))
