...
for ds in list_of_datasets:
    ...
    svm_rbf = svm.SVC(kernel='rbf', C = 1.0 , gamma='auto') #keepind default i.e auto uses 1/n_features
    count = 1
    final_results = 0
    for train_index, test_index in kf.split(X):
        ...
        pl.title("SVM - RBF (default tuning parameters) for 2 classes \n Dataset: " + str(ds) + "\n Split number:" + str(count))
        pl.savefig("SVM_RBF" + str(ds) + "split" + str(count) + '.png')
        count += 1
